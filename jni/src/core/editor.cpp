...
void cEditor_Item_Object :: draw( CEGUI::GeometryBuffer &buffer, const CEGUI::Rectf &targetRect, float alpha, const CEGUI::Rectf *clipper ) const
{
	...
	{
		m_image->render( buffer, CEGUI::Rectf(targetRect.d_min.d_x + 15, targetRect.d_min.d_y + 22, targetRect.d_min.d_x + 15 + (sprite_obj->m_start_image->m_start_w * preview_scale * global_upscalex), targetRect.d_min.d_y + 22 + (sprite_obj->m_start_image->m_start_h * preview_scale * global_upscaley) ), clipper, CEGUI::ColourRect(CEGUI::Colour(1.0f, 1.0f, 1.0f, alpha))/*, CEGUI::TopLeftToBottomRight*/ );
	}
	// name text
...
void cEditor :: Unload( void )
{
	...
	// close help window
	//pGuiSystem->getDefaultGUIContext().getRootWindow()->getChild(

	//if( CEGUI::WindowManager::getSingleton().isWindowPresent( "editor_help_window" ) )
	if(pGuiSystem->getDefaultGUIContext().getRootWindow()->getChildRecursive("editor_help_window") != 0)
	{
...
void cEditor :: Update( void )
{
	...
	if( m_listbox_menu->isVisible( /*1*/ ) )
	{
		...
		else
		{
			...
			// inactive counter
			else if( Is_Float_Equal( m_editor_window->getXPosition()./*asRelative( 1 )*/relative().d_offset, 0.0f, 0.0001f) )
			{
...
bool cEditor :: Key_Down( SDL_Keycode key )
{
	...
	// help
	//TODO disable it for now so that i dont have to fix all errors
...
void cEditor :: Unload_Item_Menu( void )
{
	...
	if(pGuiSystem->getDefaultGUIContext().getRootWindow()->getChildRecursive("editor_items") == 0)
	{
		return;
	}
...
{
	CEGUI::Window *window_help = /*CEGUI::WindowManager::getSingleton().getWindow*/pGuiSystem->getDefaultGUIContext().getRootWindow()->getChild( "editor_help_window" );
